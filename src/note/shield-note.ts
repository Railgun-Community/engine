...
export class ShieldNote {
  ...
  static decryptRandom(encryptedBundle: [string, string, string], sharedKey: Uint8Array): string {
    const decrypted = aes.gcm.decrypt(
      {
        iv: encryptedBundle[0].slice(0, 16),
        tag: encryptedBundle[0].slice(16, 32),
        data: [encryptedBundle[1].slice(0, 16)],
      },
      sharedKey,
    )[0];
  ...
  async serialize(
    ...
    const ciphertext: ShieldCiphertext = {
      encryptedBundle: [
        `${encryptedRandom.iv}${encryptedRandom.tag}`,
        combine([...encryptedRandom.data, encryptedReceiver.iv]),
