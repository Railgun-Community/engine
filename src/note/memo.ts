...
export class Memo {
  static decryptNoteAnnotationData(
    ...
    try {
      const hasTwoBytes = annotationData.length > 64;

      const metadataCiphertext = {
        iv: annotationData.substring(0, 32),
        data: hasTwoBytes
          ? [
              annotationData.substring(32, 64),
              annotationData.substring(64, 96),
              annotationData.substring(96, 128),
            ]
          : [annotationData.substring(32, 64)],
      };
