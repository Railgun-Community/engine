...
class RailgunEngine extends EventEmitter {
  ...
  async scanHistory(chain: Chain) {
    ...
    if (
      ...
      await this.clearUTXOMerkletreeAndWallets(chain);
  ...
  async syncRailgunTransactions(chain: Chain) {
    ...
    if (
      ...
      await this.clearTXIDMerkletree(chain);
