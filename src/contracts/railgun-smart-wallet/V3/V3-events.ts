...
export const formatTransactEventV3 = (
  ...
  commitmentCiphertexts: PoseidonMerkleAccumulator.CommitmentCiphertextStructOutput[],
  utxoTree: number,
  ...
  const formattedCommitments = formatTransactCommitmentsV3(
    transactionHash,
    blockNumber,
    commitmentHashes,
...
export const formatTransactCommitmentsV3 = (
  transactionHash: string,
  blockNumber: number,
  commitmentHashes: string[],
  commitmentCiphertexts: PoseidonMerkleAccumulator.CommitmentCiphertextStructOutput[],
  ...
  return commitmentCiphertexts.map((commitmentCiphertext, index) => {
    return {
      ...
      timestamp: undefined,
      blockNumber,
      ciphertext: formatCommitmentCiphertextV3(commitmentCiphertext),
      utxoTree,
...
export const formatCommitmentCiphertextV3 = (
  ...
  const iv = strippedCiphertext.slice(0, 32);
  const data = chunk(strippedCiphertext.slice(32));
...
export const formatUnshieldEventV3 = (
  transactionHash: string,
  blockNumber: number,
...
export const formatNullifiedEventsV3 = (
  transactionHash: string,
  blockNumber: number,
...
export const formatRailgunTransactionEventV3 = (
  transactionHash: string,
  blockNumber: number,
  commitments: string[],
  nullifiers: string[],
...
export const formatShieldEventV3 = (
  transactionHash: string,
  blockNumber: number,
  from: string,
  shieldPreImage: CommitmentPreimageStructOutput,
  shieldCiphertext: PoseidonMerkleAccumulator.ShieldCiphertextStructOutput,
  utxoTree: number,
  utxoIndex: number,
  fee: bigint,
): CommitmentEvent => {
  const formattedCommitment = formatShieldCommitmentV3(
    transactionHash,
    blockNumber,
    from,
    shieldPreImage,
...
export const formatShieldCommitmentV3 = (
  transactionHash: string,
  blockNumber: number,
  from: string,
  shieldPreImage: CommitmentPreimageStructOutput,
...
export const processAccumulatorUpdateEventsV3 = async (
...
export const processAccumulatorEvent = async (
  txidVersion: TXIDVersion,
  args: Result,
  transactionHash: string,
  blockNumber: number,
  ...
  try {
    ...
    for (let i = 0; i < transactions.length; i += 1) {
      ...
      const railgunTransaction = formatRailgunTransactionEventV3(
        transactionHash,
        blockNumber,
      ...
      const transactEvent = formatTransactEventV3(
        transactionHash,
        blockNumber,
        commitmentHashes,
        commitmentCiphertexts,
      ...
      const nullifiedEvents = formatNullifiedEventsV3(
        transactionHash,
        blockNumber,
      ...
      if (hasUnshield) {
        ...
        const unshieldEvent = formatUnshieldEventV3(
          transactionHash,
          blockNumber,
          unshieldPreimage,
          i, // transaction index
    ...
    for (const shield of shields) {
      ...
      const shieldEvent = formatShieldEventV3(
        transactionHash,
        blockNumber,
        from,
        preimage,
