...
describe('relay-adapt-7702', function test() {
  ...
  it('[HH] Should wrap and shield base token', async function run() {
    ...
    // Ephemeral Key
    const ephemeralWallet = deriveEphemeralWallet(config.mnemonic, 0, BigInt(chain.id), 0);
    ...
    if (!receipt) {
      throw new Error('Missing transaction receipt for 7702 shield transaction.');
    }
  ...
  it('[HH] Should execute relay adapt transaction for unshield base token', async function run() {
    ...
    expect(relayAdaptParams).to.equal(RelayAdapt7702Helper.getZeroAdaptParams());
  ...
  it('Should use zero adapt params for 7702 proofs', () => {
    expect(RelayAdapt7702Helper.getZeroAdaptParams()).to.equal(
