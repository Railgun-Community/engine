...
describe('bech32-encode', () => {
  ...
  it('Should encode and decode addresses', () => {
    for (const [index, vector] of vectors.entries()) {
      ...
      if (addressData.chain !== 'undefined') {
      ...
      if (typeof addressData.chain === 'string') {
        finalData.chain = undefined;
      }
      if (typeof addressData.version === 'string') {
        finalData.version = parseInt(addressData.version, 10);
      }
      expect(decodeAddress(encoded)).to.deep.equal(
        finalData,
        `Incorrect values for vector index ${index}`,
