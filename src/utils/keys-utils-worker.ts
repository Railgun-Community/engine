...
async function getPrivateScalarFromPrivateKey(privateKey: Uint8Array): Promise<bigint> {
  ...
  if (privateKey.length !== 32) throw new Error(`Expected 32 bytes, got ${privateKey.length} bytes`);
...
async function getSharedSymmetricKey(
  ...
  // try {
    ...
    // console.log(`                      convert Transfer: ${C - B}ms`);
    // console.log(`        getPrivateScalarFromPrivateKey: ${D - C}ms`);
    // console.log(`scalarMultiplyWasmFallbackToJavascript: ${E - D}ms`);
    // console.log(`                    sha256 and convert: ${F - E}ms`);
