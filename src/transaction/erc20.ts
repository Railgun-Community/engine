...
import { hash, bytes, babyjubjub, constants } from '../utils';
...
class ERC20Transaction {
  ...
  constructor(
    token: bytes.BytesData,
    chainID: number,
    tree: number | undefined = undefined,
  ) {
  ...
  setDepositAmountDecimal(value: string) {
    this.deposit = new BN(value, 10);
  }

  setWithdrawAmountDecimal(value: string) {
    this.withdraw = new BN(value, 10);
  }
  ...
  async generateInputs(
    ...
    const totalRequired = this.outputs
      .reduce(
        (left, right) => left.add(bytes.numberify(right.amount)),
        new BN(0),
      )
      .add(this.withdraw)
      .sub(this.deposit);
    ...
    this.outputs.forEach((output, index) => {
      if (output.token !== this.token)
        throw new Error(`TokenID mismatch on output ${index}`);
    });

    // Get UTXOs sorted by tree
    const treeSortedBalances =
      (await wallet.balancesByTree(this.chainID))[this.token] || [];
        ...
        if (treeBalance.utxos.length < length) {
          ...
          while (utxos.length < length) {
            ...
            utxos.push({
              ...
              dummyKey,
              note: new ERC20Note(
                babyjubjub.privateKeyToPublicKey(dummyKey),
                babyjubjub.random(),
                '00',
                this.token,
              ),
    ...
    for (let i = 0; i < solutions[tree].length; i += 1) {
      ...
      const privateKey =
        utxo.dummyKey ||
        wallet.getKeypair(encryptionKey, utxo.index, utxo.change, this.chainID)
          .privateKey;
      ...
      if (utxo.dummyKey) {
        ...
        pathElements.push(
          ...
          (await wallet.merkletree[this.chainID].getProof(tree, utxo.position))
            .elements,
        );
      }
    ...
    const ciphertext = commitments.map(commitment => {
      ...
      const senderPublicKey =
        babyjubjub.privateKeyToPublicKey(senderPrivateKey);
    ...
    const inputs: ERC20PrivateInputs = {
      ...
      outputTokenField:
        this.deposit.gtn(0) || this.withdraw.gtn(0) ? this.token : '00',
